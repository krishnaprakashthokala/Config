...
import org.ecommerce.config.web.models.datatables.FilterOrder;
import org.ecommerce.persistence.models.Order;
import org.ecommerce.persistence.models.Order_;
...
public class OrderFilterSpecification implements Specification<Order> {
	...
	public Predicate toPredicate(Root<Order> root, CriteriaQuery<?> cq, CriteriaBuilder cb) {
		...
		if (filterOrder.getCustomer() != null && !filterOrder.getCustomer().isEmpty()) {
			predicates.add(cb.like(cb.lower(root.get(Order_.customer).<String>get(Order_.customer.getName())),
					"%" + filterOrder.getCustomer().toLowerCase() + "%"));
		}
		...
		if (filterOrder.getPurchasedPriceFrom() != null) {
			predicates
					.add(cb.greaterThanOrEqualTo(root.get(Order_.purchasedPrice), filterOrder.getPurchasedPriceFrom()));
		}
