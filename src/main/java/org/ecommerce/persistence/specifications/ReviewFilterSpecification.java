...
import org.ecommerce.config.web.models.datatables.FilterReview;
import org.ecommerce.persistence.models.Product_;
import org.ecommerce.persistence.models.Review;
import org.ecommerce.persistence.models.Review_;
...
public class ReviewFilterSpecification implements Specification<Review> {
	...
	public Predicate toPredicate(Root<Review> root, CriteriaQuery<?> cq, CriteriaBuilder cb) {
		...
		if (filterReview.getCustumer() != null && !filterReview.getCustumer().isEmpty()) {
			predicates.add(cb.like(cb.lower(root.get(Review_.user).<String>get(User_.fullName)),
					"%" + filterReview.getCustumer().toLowerCase() + "%"));
		}
		// filter by rating
