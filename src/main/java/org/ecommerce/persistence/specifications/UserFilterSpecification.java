...
import org.ecommerce.persistence.models.User_;
...
public class UserFilterSpecification implements Specification<User> {
	...
	public Predicate toPredicate(Root<User> root, CriteriaQuery<?> cq, CriteriaBuilder cb) {
		...
		if (filterUser.getUsername() != null && !filterUser.getUsername().isEmpty()) {
			predicates.add(
					cb.like(cb.lower(root.get(User_.username)), "%" + filterUser.getUsername().toLowerCase() + "%"));
		}
		// filter by fullname
		if (filterUser.getFullname() != null && !filterUser.getFullname().isEmpty()) {
			predicates.add(
					cb.like(cb.lower(root.get(User_.fullName)), "%" + filterUser.getFullname().toLowerCase() + "%"));
		}
		// filter by last access from
		if (filterUser.getLastLoginAccessFrom() != null) {
			predicates
					.add(cb.greaterThanOrEqualTo(root.get(User_.lastLoginAccess), filterUser.getLastLoginAccessFrom()));
		}
