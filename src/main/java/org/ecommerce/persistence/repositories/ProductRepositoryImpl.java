...
public class ProductRepositoryImpl extends AbstractSearchableJpaRepository<Product> {
	...
	public Page<Product> search(String query, Pageable pageable) {
		...
		QueryBuilder builder = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(Product.class)
				.get();

		Query lucene = builder.keyword().onFields("name", "description", "shortDescription").matching(query)
				.createQuery();
